...
class ExpressionMaskTests(unittest.TestCase):
    ...
    def test_add(self):
        ...
        assert AddOperator(IntLiteral(0xFC00), IntLiteral(0x300)).mask == \
            0xFF00
        assert AddOperator(IntLiteral(0xFF00), IntLiteral(0x300)).mask == \
            0x1FF00
        assert AddOperator(IntLiteral(0xFF00FF), IntLiteral(0xFF00FF)).mask == \
            0x1FF01FF
    ...
    def test_lvshift(self):
        ...
        assert LVShift(v8, TestValue('C', IntType.u(3))).mask == \
            0x7FFF
        # Test expr masks with gaps.
        assert LVShift(OrOperator(v1, LShift(v1, 4)), v1).mask == \
            0x33
        assert LVShift(OrOperator(LShift(v1, 2), LShift(v1, 4)), v1).mask == \
            0x3C
        # Test offset masks with gaps.
        assert LVShift(v8, OrOperator(v1, LShift(v1, 4))).mask == \
            0x1FF01FF
        assert LVShift(v8, OrOperator(LShift(v1, 2), LShift(v1, 4))).mask == \
            0xFFF0FFF
        ...
        assert LVShift(OrOperator(LShift(i, 32), v8), LShift(v1, 4)).mask == \
            (-1 << 32) | 0xFF00FF
        # Test negative offset mask.
        assert LVShift(v8, i).mask == -1
        assert LVShift(v8, LShift(i, 4)).mask == (-1 << 16) | 0xFF
        assert LVShift(LShift(v8, 16), LShift(i, 4)).mask == \
            (-1 << 32) | 0xFF0000
    ...
    def test_rvshift(self):
        ...
        # Test generic variable mask.
        assert RVShift(v8, TestValue('C', IntType.u(3))).mask == \
            0xFF
        # Test expr masks with gaps.
        assert RVShift(OrOperator(v1, LShift(v1, 4)), v1).mask == \
            0x19
        assert RVShift(OrOperator(LShift(v1, 2), LShift(v1, 4)), v1).mask == \
            0x1E
        # Test offset masks with gaps.
        assert RVShift(LShift(v8, 20), OrOperator(v1, LShift(v1, 4))).mask == \
            0xFF80FF8
        assert RVShift(LShift(v8, 16), OrOperator(v1, LShift(v1, 4))).mask == \
            0xFF80FF
        # Test gaps on both args.
        assert RVShift(
